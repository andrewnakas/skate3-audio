...
| Audio in the Rust engine | `crates/skate-data/src/audio/` decodes a retail archive member **byte-exactly** (checked against an independent reference on real data) and a Bevy source in `crates/skate-game/src/skate_audio.rs` feeds it to the mixer. **Not yet heard** -- that checkout has no set-up assets, so the game does not boot there; and choosing a stream by name still needs the undecoded metadata table |
