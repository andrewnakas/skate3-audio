...
| Container formats | all three decoded and documented; `.mpf` sequencing decoded including sections 0–3; the `.abk`/`.bnk`/`.csi`/`.ems` banks decoded far enough to show that **a 64-bit name hash is the join key** between code, world data and banks (`docs/audio-banks.md`) |
...
| Audio in the Rust engine | **plays real streams.** `crates/skate-data/src/audio/` decodes a retail archive member **byte-exactly** (against an independent reference, on real data) and the Bevy source in `crates/skate-game/src/skate_audio.rs` feeds it to the mixer. Confirmed at the sound card: recording the output device while `skate3-audio-check` played the 5-channel ambience bed, the decoded waveform cross-correlates with the recording at lag 1.370 s, peak/mean 58.8 against 5.7 for a time-reversed control. Choosing a stream by name still needs the undecoded metadata table |
| Rust crates | **two**. `skate-audio-formats`: 33 tests, three containers, validated on real archives. `skate-audio-core`: **132 of the 140 verified (or thin) C++ bodies have a Rust translation with a replay arm**, plus the helpers that blocked them, in 614 unit tests. Every recorded call replays with **zero failures**: 109,099 through 2026-09-13, then about 11,000 more from the 65-function `backlog1` session on 2026-09-14. The engine only needs the player character's sounds (user direction, 2026-09-14), so music- and ambience-only work comes off the Rust queue. The per-module figures, and which ports are unit-tested only, are in `rust/skate-audio-core/README.md`; read the limits in `docs/shadow-harness.md` before quoting a number |
