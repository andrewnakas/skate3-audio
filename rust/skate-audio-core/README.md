...
| `patch.rs` | `sub_828DC660`'s bank branch, `sub_82B1DF50`, `sub_828E2B48`, `sub_82B1DAD0`, `sub_82B1D880`, `sub_828E2FF8` and four instance callbacks: load and install an `.abk`, post a message, spawn an evaluator instance | **unverified new work** (loader and game threads, no C++ body). Unit-tested; `examples/grind_instance.rs` runs the real `GRINDS.abk` over the dumped image until the first unported opcode, slot 27 |
...
`cargo test` runs 634 unit tests. **Read the next two sections before reading that as one number:
