...
| `mathlib.rs` | `sub_82F4DE80` (`floor`, 2.87 M calls a boot — the hottest body ported anywhere in this project), `Image`'s sine (`sub_82F4DED0`) and cosine (`sub_82F4DFB0`), and `log10` (`sub_82F55068`) with the natural log `sub_82F54ED8` under it — the last three ported beyond the 216 because callers here needed them | **7,000 replayed** (2,000 floor + 999 sine + 1,001 cosine + 3,000 log10), all compared by the bits of the returned `f1` |
