...
| `pitch.rs` | `sub_82B2DAC8`: a voice's pitch ratio recomputed only when it moved, turned into a 16.16 step rounded half away from zero and clamped at 2^18, then the block's source frames counted — a 64-bit result whose clamp looks at the low word only | unit-tested, recording pending |
...
`cargo test` runs 598 unit tests. **Read the next two sections before reading that as one number:
