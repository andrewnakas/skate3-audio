...
| `leaves.rs` | ten small verified leaves that belong to no larger mechanism ported yet — four command-ring handlers (`sub_82B463A8`, `sub_82B49268`, `sub_82B34B10`, `sub_82B23828`), a `u16` field setter, a 64-bit identity, a stream's unconsumed byte count, a record-size calculation, a two-field zeroing, and a copy that marks its block filled | **13,260 replayed** (3,232 + 230 + 230 + 2,969 + 2,619 + 1,404 + 11 + 924 + 397 + 1,241, and 3 for `sub_82B29278`); `sub_82B2FE00`, the thin five-point ramp, is unit-tested only — its only vectors predate read spans |
...
`cargo test` runs 523 unit tests. **Read the next two sections before reading that as one number:
