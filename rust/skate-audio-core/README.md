...
| `routing.rs` | `sub_82B426D0`, the scatter-mixer: run a table of route bytes — the first route to a destination overwrites it and later ones accumulate — then zero every destination slot no route wrote. Composes `dsp::scale`'s two kernels and `mem::memset` | **500 replayed** |
